...
inline int RubiksCube1dArray::getIndex(int ind, int row, int col) {
...
RubiksCube::COLOR RubiksCube1dArray::getColor(RubiksCube::FACE face, unsigned row, unsigned col) const  {
...
bool RubiksCube1dArray::isSolved() const  {
    for (int i = 0; i < 6; i++) {
        for (int j = 0; j < 3; j++) {
            for (int k = 0; k < 3; k++) {
                if (this->cube[getIndex(i, j, k)] == getColorLetter(RubiksCube::COLOR(i))) continue;
                return false;
            }
        }
    }
